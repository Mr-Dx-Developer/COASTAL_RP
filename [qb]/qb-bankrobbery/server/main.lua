        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == 'item' then
                    ...
                    exports['qs-inventory']:AddItem(src, item.item, itemAmount, false, false, 'qb-bankrobbery:server:recieveItem')
                    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[item.item], 'add')
                elseif Config.RewardTypes[itemType].type == 'money' then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    exports['qs-inventory']:AddItem(src, 'markedbills', math.random(2, 3), false, info, 'qb-bankrobbery:server:recieveItem')
                    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['qs-inventory']:AddItem(src, 'security_card_01', 1, false, false, 'qb-bankrobbery:server:recieveItem')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['security_card_01'], 'add')
            end
        else
            exports['qs-inventory']:AddItem(src, 'weapon_stungun', 1, false, false, 'qb-bankrobbery:server:recieveItem')
            TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['weapon_stungun'], 'add')
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == 'money' then
                    ...
                    exports['qs-inventory']:AddItem(src, 'markedbills', math.random(1, 4), false, info, 'qb-bankrobbery:server:recieveItem')
                    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['qs-inventory']:AddItem(src, 'security_card_02', 1, false, false, 'qb-bankrobbery:server:recieveItem')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['security_card_02'], 'add')
            end
        else
            exports['qs-inventory']:AddItem(src, 'weapon_vintagepistol', 1, false, false, 'qb-bankrobbery:server:recieveItem')
            TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['weapon_vintagepistol'], 'add')
        end
    elseif type == 'pacific' then
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                exports['qs-inventory']:AddItem(src, 'cryptostick', 1, false, info, 'qb-bankrobbery:server:recieveItem')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['cryptostick'], 'add')
            end
        else
            local chance = math.random(1, 2)
            local odd = math.random(1, 2)
            if chance == odd then
                exports['qs-inventory']:AddItem(src, 'weapon_microsmg', 1, false, false, 'qb-bankrobbery:server:recieveItem')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['weapon_microsmg'], 'add')
            else
                exports['qs-inventory']:AddItem(src, 'weapon_minismg', 1, false, false, 'qb-bankrobbery:server:recieveItem')
                TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['weapon_minismg'], 'add')
            end
        end
    end
end)
...
RegisterNetEvent('qb-bankrobbery:server:removeElectronicKit', function()
    ...
    exports['qs-inventory']:RemoveItem(src, 'electronickit', 1, false, 'qb-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['electronickit'], 'remove')
    exports['qs-inventory']:RemoveItem(src, 'trojan_usb', 1, false, 'qb-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['trojan_usb'], 'remove')
end)
...
RegisterNetEvent('qb-bankrobbery:server:removeBankCard', function(number)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['qs-inventory']:RemoveItem(src, 'security_card_' .. number, 1, false, 'qb-bankrobbery:server:removeBankCard')
    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['security_card_' .. number], 'remove')
end)
...
QBCore.Functions.CreateCallback('thermite:server:check', function(source, cb)
    ...
    if exports['qs-inventory']:RemoveItem(source, 'thermite', 1, false, 'thermite:server:check') then
        TriggerClientEvent('qs-inventory:client:ItemBox', source, QBCore.Shared.Items['thermite'], 'remove')
        cb(true)
    else
        cb(false)
    end
