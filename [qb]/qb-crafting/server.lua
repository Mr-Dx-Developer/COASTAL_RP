...
RegisterServerEvent('qb-crafting:server:removeMaterials', function(itemName, amount)
    ...
    if Player then
        exports['qs-inventory']:RemoveItem(src, itemName, amount, false, 'qb-crafting:server:removeMaterials')
        TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
    end
end)
...
RegisterNetEvent('qb-crafting:server:removeCraftingTable', function(benchType)
    ...
    exports['qs-inventory']:RemoveItem(src, benchType, 1, false, 'qb-crafting:server:removeCraftingTable')
    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[benchType], 'remove')
...
RegisterNetEvent('qb-crafting:server:addCraftingTable', function(benchType)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if not exports['qs-inventory']:AddItem(src, benchType, 1, false, false, 'qb-crafting:server:addCraftingTable') then return end
    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[benchType], 'add')
end)
...
RegisterNetEvent('qb-crafting:server:receiveItem', function(craftedItem, requiredItems, amountToCraft, xpGain, xpType)
    ...
    for _, requiredItem in ipairs(requiredItems) do
        if not exports['qs-inventory']:RemoveItem(src, requiredItem.item, requiredItem.amount, false, 'qb-crafting:server:receiveItem') then
            canGive = false
            return
        end
        TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[requiredItem.item], 'remove')
    end
    if canGive then
        if not exports['qs-inventory']:AddItem(src, craftedItem, amountToCraft, false, false, 'qb-crafting:server:receiveItem') then return end
        TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[craftedItem], 'add')
