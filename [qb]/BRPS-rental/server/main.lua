...
RegisterNetEvent('BRPS-rental:sv:rentVehicle', function(data)
    local src = source
    ...
    if data.payType == "cash" then
        if Player.Functions.GetMoney("cash") >= data.carPrice then
            ...
            TriggerClientEvent('BRPS-rental:cl:spawnVehicle', src, data.carName, data.carDay)
...
RegisterNetEvent('BRPS-rental:sv:updatesql', function(plate,vehicle,time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
...
RegisterNetEvent('BRPS-rental:sv:checktime', function()
