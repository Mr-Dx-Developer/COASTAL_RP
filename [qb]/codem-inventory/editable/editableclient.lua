...
function CheckVehicleInventory()
    ...
    if InGlovebox then
        ...
        InGlovebox               = customToLower(InGlovebox)
    ...
    if curvehicle then
        ...
        currentVehiclePlate = customToLower(currentVehiclePlate)
...
RegisterNetEvent('codem-inventory:OpenPlayerShop', function(items)
    ...
    NuiMessage('OPEN_SHOP', { shopname = 'market', shoplabel = 'MARKET', inventory = items })
end)
...
function spawnPed()
    for k, v in pairs(Config.RepairWeapon) do
        if v and v.ped then
            ...
            local createNpc = CreatePed("PED_TYPE_PROSTITUTE", v.pedHash, v.coords.x,
                v.coords.y,
                v.coords.z - 0.98, v.heading, false, false)
