...
AddEventHandler("playerDropped", function()
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		--if player.Source ~= src then
			TriggerClientEvent("BRPS-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
		--end
	end
...
function Connect(src, currentRadioChannel, radioChannelToJoin)
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("BRPS-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, radioChannelToJoin, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
end
...
function Disconnect(src, currentRadioChannel) 
	local playersInCurrentRadioChannel = CreateFullRadioListOfChannel(currentRadioChannel)
	TriggerClientEvent("BRPS-RadioList:Client:SyncRadioChannelPlayers", src, src, 0, playersInCurrentRadioChannel)
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("BRPS-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
