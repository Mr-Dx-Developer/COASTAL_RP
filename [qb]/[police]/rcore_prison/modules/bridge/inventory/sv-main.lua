...
Bridge.AddItem = function(playerId, itemName, itemCount, metadata, slot)
    ...
    if state then
        ...
        elseif HasResource('tgiann-inventory') then
            ...
            if itemName == 'money' and Prison.Framework == FRAMEWORK_MAP.ESX then
                return Player.addMoney(tonumber(itemCount), 'add_item')
            end

            if itemName == 'money' and Prison.Framework == FRAMEWORK_MAP.QBCORE then
                return Player.Functions.AddMoney('cash', itemCount)
            end
    
            local retval = exports['tgiann-inventory']:CanGiveItems(playerId, itemName, itemCount)
    
            if retval then
                return exports['tgiann-inventory']:AddItem(playerId, itemName, itemCount, nil, metadata or {}, false)
            end
...
Bridge.RemoveItem = function(playerId, itemName, itemCount, metadata, slot)
    ...
    if state then
        ...
        elseif HasResource('tgiann-inventory') then
            ...
            return exports['tgiann-inventory']:RemoveItem(playerId, itemName, itemCount, nil)
        elseif HasResource(Prison.InventoryScripts.MF_INVENTORY) and Prison.Framework == FRAMEWORK_MAP.ESX then
    ...
    if state then
        ...
        if Prison.Framework == FRAMEWORK_MAP.ESX then
            ...
            if hasOxInventory then
                return exports.ox_inventory:GetItemCount(playerId, itemName, {}, false)
            end
        ...
        elseif HasResource('tgiann-inventory') then
            ...
            local item = exports['tgiann-inventory']:GetItemByName(playerId, itemName) or {}
            local ItemInfo = {
                name = itemName,
...
Bridge.ReturnPrisonerItemsFromStash = function(src)
    dbg.debugInventory('Returning items from stash for player %s!', GetPlayerName(src))
...
function GetServerInventory()
    ...
    if HasResource('tgiann-inventory') then
        DeployInventory = 'tgiann-inventory'
    end
...
function getInventoryDataByPlayerId(serverId)
    ...
    for resourceName, scriptName in pairs(Prison.InventoryScripts) do
        ...
        if HasResource('tgiann-inventory') then
            local inventory = exports["tgiann-inventory"]:GetPlayerItems(serverId)
            return p:resolve(inventory)
        end
