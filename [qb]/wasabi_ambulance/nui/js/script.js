...
window.addEventListener('message', function(event) {
    if (event.data.action === "displayDeathScreen") {
        ...
        if (event.data.type === 1) {
            displayScreenOne(event.data, color, translationStrings);
        } else if (event.data.type === 2) {
            displayScreenTwo(event.data, color, translationStrings)
        }
        else if (event.data.type === 3) {
            displayScreenThree(event.data, color, translationStrings)
        }
        else if (event.data.type === 4) {
            displayScreenFour(event.data, color, translationStrings)
        }
    }
});
