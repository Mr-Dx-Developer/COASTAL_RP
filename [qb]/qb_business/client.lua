		...
		for k,mark in pairs(Config.empresas) do
			...
			if not menuactive and distance <= 20.0 then
				...
				if distance <= 1.5 then
					DrawText3D2(x,y,z-0.6, Lang[Config.lang]['open']:format(mark.valor_compra), 0.40)
		...
		if distance <= 50.0 then
			...
			if distance <= 4.0 then
				if veh == nveh2 then
					...
					DoScreenFadeIn(1000)
					Citizen.CreateThreadNow(function()
						showScaleform(Lang[Config.lang]['sucess'], Lang[Config.lang]['products_sold_cli'], 3)
					end)
					nveh2 = nil
					blip2 = nil
...
function spawnVehicle2(name,x,y,z,h)
	...
	if HasModelLoaded(mhash) then
		...
		SetVehicleNumberPlateText(NetToVeh(netveh),Lang[Config.lang]['vehicle_plate'])
		TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(nveh2))
		Citizen.InvokeNative(0xAD738C3085FE7E11,NetToVeh(netveh),true,true)
		SetVehicleHasBeenOwnedByPlayer(NetToVeh(netveh),true)
		SetVehicleNeedsToBeHotwired(NetToVeh(netveh),false)
		SetModelAsNoLongerNeeded(mhash)
...
function SpawnEnemyNPC(x, y, z, target) -- Works decent but not exactly how I want, still working no improving it
	...
	if done and HasModelLoaded(0x964D12DC) and HasModelLoaded(0x132D5A1A) then
		...
		GiveWeaponToPed(Driver, "WEAPON_PISTOL", 400, false, true)	GiveWeaponToPed(Passenger, "WEAPON_PISTOL", 400, false, true)
