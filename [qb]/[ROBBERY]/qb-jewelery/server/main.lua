    ...
    if cachedPoliceAmount[source] >= Config.RequiredCops then
        if plrPed then
            ...
            if dist <= 25.0 then
                ...
                if exports['qs-inventory']:AddItem(src, Config.VitrineRewards[item]['item'], amount, false, false, 'qb-jewellery:server:vitrineReward') then
                    TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items[Config.VitrineRewards[item]['item']], 'add')
                else
