...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        ...
        exports['qs-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['stickynote'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    if luck <= 10 then
        exports['qs-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'qb-storerobbery:server:SafeReward')
...
RegisterNetEvent('qb-storerobbery:server:removeAdvancedLockpick', function()
    ...
    if not Player then return end
    exports['qs-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'qb-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('qb-storerobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
    exports['qs-inventory']:RemoveItem(source, 'lockpick', 1, false, 'qb-storerobbery:server:removeLockpick')
end)
