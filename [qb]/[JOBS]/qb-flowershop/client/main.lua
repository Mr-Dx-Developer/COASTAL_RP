...
CreateThread(function()
    for k, flowerProcessing in pairs(Config.Locations["flowerProcessing"]) do
        local blip = AddBlipForCoord(flowerProcessing.coords.x, flowerProcessing.coords.y, flowerProcessing.coords.z)
        SetBlipSprite(blip, 642)
        SetBlipAsShortRange(blip, true)
        SetBlipScale(blip, 0.6)
        SetBlipColour(blip, 37)
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString(flowerProcessing.label)
        EndTextCommandSetBlipName(blip)
    end
end)
