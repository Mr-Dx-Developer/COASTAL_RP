...
RegisterNetEvent('equip:harness', function(item)
    ...
    elseif Player.PlayerData.items[item.slot].info.uses == 1 then
        exports['qs-inventory']:RemoveItem(src, 'harness', 1, false, 'equip:harness')
        TriggerClientEvent('qs-inventory:client:ItemBox', src, QBCore.Shared.Items['harness'], 'remove')
    else
...
RegisterNetEvent('seatbelt:DoHarnessDamage', function(hp, data)
    ...
    if not Player then return end
    if hp == 0 then
        exports['qs-inventory']:RemoveItem(src, 'harness', 1, data.slot, 'seatbelt:DoHarnessDamage')
    else
