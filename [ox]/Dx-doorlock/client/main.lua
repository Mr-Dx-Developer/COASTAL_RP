...
local function displayNUIText(text)
	...
	-- })
	if text == "مقفل" then
		exports['l2s-interface']:Show('Locked', '<i class="fas fa-lock"></i>')
	elseif text == "مفتوح" then
		exports['l2s-interface']:Show('Unlocked', '<i class="fas fa-unlock"></i>')
	elseif text == "مفتوح1" then
		exports['l2s-interface']:Show('Unlocked', '<i class="fas fa-unlock"></i>')
	elseif text == "مقفل1" then
		exports['l2s-interface']:Show('Locked', '<i class="fas fa-lock"></i>')
	end
...
local function playSound(door, src, enableSounds)
	...
	if distance < 10 then
		if door.audioRemote  or door.slides then
		...
		if not door.audioRemote and not door.slides then
			SendNUIMessage({
				type = 'audio',
				audio = sound,
				distance = distance,
        ...
        if not specificDoor or doorID == specificDoor then
            ...
            if not data.setText and data.doors then
                for k, v in pairs(data.doors) do
                    ...
                    if k == 2 and data.objCoords and (data.doorType == "slides" or data.doorType == "garage") then
                        if GetEntityHeightAboveGround(v.object) < 1 then
                            -- data.objCoords = vec3(data.objCoords.x, data.objCoords.y, data.objCoords.z + 1.2)
							data.objCoords = vec3(data.objCoords.x, data.objCoords.y, data.objCoords.z+1.2)
                        end
                    end
                end
            elseif not data.setText and not data.doors and DoesEntityExist(data.object) then
                ...
                if data.doorType == "slides" or data.doorType == "garage" then
                    if GetEntityHeightAboveGround(data.object) < 1 then
                        data.objCoords = vec3(data.objCoords.x, data.objCoords.y, data.objCoords.z + 1.6)
                    end
                end
            end
        end
...
local function lockpickFinish(success)
	if success then
		...
		Wait(1800)
		TriggerServerEvent('l2s-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('l2s-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('l2s-doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('l2s-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds, togggllee)
	...
	if not Config.DoorList[doorID].slides then
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	TriggerEvent('l2s-lockpick:client:openLockpick', lockpickFinish)
end)

RegisterNetEvent('lockpicks:UsePoliceLockpick', function(isAdvanced)
...
RegisterNetEvent('l2s-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	local doorData = {}
	local dialog = exports['l2s-interface']:ShowInput({
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		TriggerServerEvent('l2s-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('l2s-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('l2s-doorlock:client:newDoorAdded', function(data, id)
	Config.DoorList[id] = data
	TriggerEvent('l2s-doorlock:client:setState', PlayerData.source, id, data.locked, false, true, true)
end)
...
RegisterCommand('toggledoorlock', function()
	...
	if Config.DoorList[closestDoor.id].slides then
		local HasItem = exports['l2s-inventory']:HasItem('keyfob') 
		if HasItem or Config.DoorList[closestDoor.id].mazen  then
			TriggerServerEvent('l2s-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
		else
			PlaySoundFromEntity(-1, "Keycard_Fail", PlayerPedId(), "DLC_HEISTS_BIOLAB_FINALE_SOUNDS", 1, 5.0);
			QBCore.Functions.Notify("You dont have a keyfob...", 'error')   
			return -- End the loop
		end
	else
		TriggerServerEvent('l2s-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
			...
			if closestDoor.id then
				...
				while isLoggedIn do
					...
					elseif paused then
						...
						playerCoords = GetEntityCoords(playerPed)
						closestDoor.distance = #(closestDoor.data.objCoords - playerCoords)
